...
class GameAPI {
public:
    explicit GameAPI(ScriptEngine& engine);
...
private:
    ...
    // Timer namespace
    void api_timer_after(double delaySec, sol::function callback);
    void api_timer_every(double intervalSec, sol::function callback);
    void api_timer_named(const std::string& name, double delaySec, sol::function callback);
    ...
    // Cached callback names for timers (since we can't store sol::function directly)
    std::unordered_map<std::string, std::string> timerCallbacks_;

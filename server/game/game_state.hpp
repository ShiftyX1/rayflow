...
public:
    ...
    // Add player to match
    PlayerState* add_player(shared::proto::PlayerId playerId, const std::string& name);
    
    // Remove player from match
    void remove_player(shared::proto::PlayerId playerId);
    
    // Get player state
    PlayerState* get_player(shared::proto::PlayerId playerId);
    const PlayerState* get_player(shared::proto::PlayerId playerId) const;
    ...
    shared::game::TeamId assign_player_to_team(shared::proto::PlayerId playerId,
                                                shared::game::TeamId preferred = shared::game::Teams::None);
    ...
    // Find bed by team
    Bed* get_team_bed(shared::game::TeamId teamId);
    ...
    // End the match
    void end_match(shared::game::TeamId winner);
    ...
    std::uint8_t process_attack(shared::proto::PlayerId attacker, 
                                shared::proto::PlayerId target,
                                std::uint64_t tick);
    
    // Process player death
    void process_death(shared::proto::PlayerId playerId, std::uint64_t tick);
    
    // Process respawn request
    bool try_respawn(shared::proto::PlayerId playerId, std::uint64_t tick);
    
    // === Bed ===
    
    // Process bed destruction
    void process_bed_destruction(int x, int y, int z, 
                                  shared::proto::PlayerId destroyer,
                                  std::uint64_t tick);
    ...
    void process_item_pickup(shared::proto::PlayerId playerId);
    
    // === Events (callbacks for networking) ===
    
    std::function<void(shared::proto::PlayerId, const std::string&)> onPlayerMessage;
    std::function<void(const std::string&)> onBroadcast;
    std::function<void(shared::proto::PlayerId, std::uint8_t, std::uint8_t)> onHealthChanged;
    std::function<void(shared::proto::PlayerId, shared::proto::PlayerId)> onPlayerKilled;
    std::function<void(shared::proto::PlayerId, float, float, float)> onPlayerRespawned;
    std::function<void(shared::game::TeamId, shared::proto::PlayerId)> onBedDestroyed;
    std::function<void(shared::game::TeamId)> onTeamEliminated;
    std::function<void(shared::game::TeamId)> onMatchEnded;
...
private:
    ...
    shared::game::TeamId winner_{shared::game::Teams::None};
};

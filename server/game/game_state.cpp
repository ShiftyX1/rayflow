...
void GameState::init(const MatchConfig& config) {
    ...
    matchTick_ = 0;
    phaseStartTick_ = 0;
    winner_ = shared::game::Teams::None;
}
...
PlayerState* GameState::add_player(shared::proto::PlayerId playerId, const std::string& name) {
...
void GameState::remove_player(shared::proto::PlayerId playerId) {
    // Remove from team
...
PlayerState* GameState::get_player(shared::proto::PlayerId playerId) {
    for (auto& p : players_) {
...
const PlayerState* GameState::get_player(shared::proto::PlayerId playerId) const {
...
shared::game::TeamId GameState::assign_player_to_team(shared::proto::PlayerId playerId,
                                                       shared::game::TeamId preferred) {
...
Bed* GameState::get_team_bed(shared::game::TeamId teamId) {
    for (auto& bed : beds_) {
...
void GameState::end_match(shared::game::TeamId winner) {
    winner_ = winner;
...
void GameState::check_win_condition() {
    ...
    if (winner != shared::game::Teams::None) {
        end_match(winner);
    }
}

std::uint8_t GameState::process_attack(shared::proto::PlayerId attacker, 
...
void GameState::process_death(shared::proto::PlayerId playerId, std::uint64_t tick) {
...
bool GameState::try_respawn(shared::proto::PlayerId playerId, std::uint64_t tick) {
    auto* player = get_player(playerId);
...
void GameState::process_item_pickup(shared::proto::PlayerId playerId) {
    auto* player = get_player(playerId);
    ...
    for (auto itemId : nearby) {
        ...
        // Add to inventory
        if (shared::game::is_resource(item->itemType)) {

...
shared::game::ItemType Generator::item_type() const {
    return shared::game::generator_item_type(tier);
}
...
EntityId GeneratorManager::create_generator(float x, float y, float z, 
    ...
    gen.z = z;
    gen.tier = tier;
    gen.spawnInterval = shared::game::default_spawn_interval(tier);
    gen.timeUntilSpawn = gen.spawnInterval;
    gen.maxItems = shared::game::generator_max_items(tier);
...
void GeneratorManager::upgrade_team_generators(shared::game::TeamId team, std::uint8_t level) {
    for (auto& gen : generators_) {

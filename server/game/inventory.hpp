...
struct InventorySlot {
    shared::game::ItemType item{shared::game::ItemType::None};
    std::uint16_t count{0};
    
    bool is_empty() const { return item == shared::game::ItemType::None || count == 0; }
    
    void clear() {
        item = shared::game::ItemType::None;
        count = 0;
    }
};
...
public:
    ...
    std::uint32_t get_resource_count(shared::game::ItemType type) const;
    
    // Add resources, returns amount actually added
    std::uint32_t add_resource(shared::game::ItemType type, std::uint32_t amount);
    
    // Remove resources, returns true if successful
    bool remove_resource(shared::game::ItemType type, std::uint32_t amount);
    
    // Check if player has enough resources
    bool has_resource(shared::game::ItemType type, std::uint32_t amount) const;
    ...
    void set_hotbar_slot(std::size_t slot, shared::game::ItemType item, std::uint16_t count = 1);
    
    // Add item to first available hotbar slot, returns slot index or -1 if full
    int add_to_hotbar(shared::game::ItemType item, std::uint16_t count = 1);
    ...
    int find_item(shared::game::ItemType item) const;
    
    // Check if hotbar has item
    bool has_item(shared::game::ItemType item) const;
    ...
    shared::game::ItemType get_armor(ArmorSlotIndex slot) const;
    
    // Set armor in slot (old armor is lost)
    void set_armor(ArmorSlotIndex slot, shared::game::ItemType item);
...
private:
    ...
    std::array<shared::game::ItemType, ArmorSlots> armor_{};

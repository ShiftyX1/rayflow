...
bool UIScriptEngine::init() {
    ...
    auto config = engine::scripting::SandboxConfig::default_for_ui();
    ...
    lua_ = engine::scripting::Sandbox::create(config);
    if (!lua_) {
...
engine::scripting::ScriptResult UIScriptEngine::load_script(const std::string& script, const std::string& name) {
    if (!lua_) {
        return engine::scripting::ScriptResult::fail("Engine not initialized");
    }
    ...
    if (!validation.valid) {
        ...
        return engine::scripting::ScriptResult::fail(lastError_);
    }
    ...
    return engine::scripting::ScriptResult::ok();
}

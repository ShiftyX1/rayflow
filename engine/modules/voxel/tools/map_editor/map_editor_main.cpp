...
#include "../../client/block_interaction.hpp"
#include "../../client/block_model_loader.hpp"
#include "../../client/block_registry.hpp"
...
int main() {
    ...
    std::shared_ptr<engine::transport::IServerTransport> serverTransport;
    std::unique_ptr<engine::ServerEngine> serverEngine;
    std::unique_ptr<bedwars::server::BedWarsServer> serverGame;
    std::thread serverThread;
    std::unique_ptr<editor::EditorSession> session;
    std::unique_ptr<voxel::World> world;

    std::optional<bedwars::proto::ActionRejected> lastReject;
    std::optional<bedwars::proto::ExportResult> lastExport;
    ...
    const auto enter_editor = [&]() {
        TraceLog(LOG_INFO, "[editor] enter_editor: Creating transport pair");
        
        // Create local transport pair (client <-> server)
        auto enginePair = engine::transport::create_local_transport_pair();
        clientTransport = enginePair.client;
        serverTransport = enginePair.server;

        // Configure and create server engine
        engine::ServerEngine::Config serverConfig;
        serverConfig.tickRate = 30;
        serverConfig.logging = true;  // Enable for debugging

        serverEngine = std::make_unique<engine::ServerEngine>(serverConfig);
        serverEngine->set_transport(serverTransport);

        // Create BedWars server with editor options
        ...
        TraceLog(LOG_INFO, "[editor] enter_editor: Starting server thread");
        ...
        TraceLog(LOG_INFO, "[editor] enter_editor: Creating EditorSession");

        // Create editor session (direct engine transport, no legacy adapter)
        session = std::make_unique<editor::EditorSession>(clientTransport);
        ...
        session->set_on_block_placed([&](const bedwars::proto::BlockPlaced& ev) {
            if (!world) return;
        ...
        session->set_on_block_broken([&](const bedwars::proto::BlockBroken& ev) {
            if (!world) return;
            world->set_block(ev.x, ev.y, ev.z, static_cast<voxel::Block>(voxel::BlockType::Air));
        });
        session->set_on_action_rejected([&](const bedwars::proto::ActionRejected& ev) {
            lastReject = ev;
        });
        session->set_on_export_result([&](const bedwars::proto::ExportResult& ev) {
            lastExport = ev;
        });

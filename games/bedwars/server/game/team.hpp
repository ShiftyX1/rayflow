...
struct Team {
    bedwars::game::TeamId id{bedwars::game::Teams::None};
    std::string name;
    bedwars::game::TeamColor color;
    ...
    // Members
    std::vector<engine::PlayerId> memberIds;
    ...
    bool has_member(engine::PlayerId playerId) const;
    void add_member(engine::PlayerId playerId);
    void remove_member(engine::PlayerId playerId);
...
public:
    ...
    // Get team by ID
    Team* get_team(bedwars::game::TeamId id);
    const Team* get_team(bedwars::game::TeamId id) const;
    
    // Get team for a player
    Team* get_player_team(engine::PlayerId playerId);
    const Team* get_player_team(engine::PlayerId playerId) const;
    
    // Assign player to team (auto-balance or specific)
    bedwars::game::TeamId assign_player(engine::PlayerId playerId, 
                                        bedwars::game::TeamId preferredTeam = bedwars::game::Teams::None);
    
    // Remove player from their team
    void remove_player(engine::PlayerId playerId);
    
    // Get team with fewest members (for auto-balance)
    bedwars::game::TeamId get_smallest_team() const;
    
    // Count non-eliminated teams
    std::size_t alive_team_count() const;
    
    // Get winning team (if only one remains)
    bedwars::game::TeamId get_winner() const;

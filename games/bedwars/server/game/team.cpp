...
bool Team::has_member(engine::PlayerId playerId) const {
    return std::find(memberIds.begin(), memberIds.end(), playerId) != memberIds.end();
}

void Team::add_member(engine::PlayerId playerId) {
    if (!has_member(playerId)) {
        memberIds.push_back(playerId);
    }
}

void Team::remove_member(engine::PlayerId playerId) {
    memberIds.erase(
...
void TeamManager::init_teams(std::size_t teamCount) {
    ...
    const bedwars::game::TeamId teamIds[] = {
        bedwars::game::Teams::Red,
        bedwars::game::Teams::Blue,
        bedwars::game::Teams::Green,
        bedwars::game::Teams::Yellow,
        bedwars::game::Teams::Aqua,
        bedwars::game::Teams::White,
        bedwars::game::Teams::Pink,
        bedwars::game::Teams::Gray,
    };
    
    for (std::size_t i = 0; i < teamCount && i < 8; ++i) {
        Team team;
        team.id = teamIds[i];
        team.name = bedwars::game::team_name(team.id);
        team.color = bedwars::game::TeamColor::from_team_id(team.id);
        teams_.push_back(std::move(team));
    }
}

Team* TeamManager::get_team(bedwars::game::TeamId id) {
    for (auto& t : teams_) {
        if (t.id == id) return &t;
    }
    return nullptr;
...
const Team* TeamManager::get_team(bedwars::game::TeamId id) const {
...
Team* TeamManager::get_player_team(engine::PlayerId playerId) {
    for (auto& t : teams_) {
...
const Team* TeamManager::get_player_team(engine::PlayerId playerId) const {
    for (const auto& t : teams_) {
        if (t.has_member(playerId)) return &t;
    }
    return nullptr;
...
bedwars::game::TeamId TeamManager::assign_player(engine::PlayerId playerId, 
                                                 bedwars::game::TeamId preferredTeam) {
    // First, remove from any existing team
    remove_player(playerId);
    
    Team* targetTeam = nullptr;
    
    if (preferredTeam != bedwars::game::Teams::None) {
    ...
    return bedwars::game::Teams::None;
}

void TeamManager::remove_player(engine::PlayerId playerId) {
    for (auto& t : teams_) {
        t.remove_member(playerId);
    }
}

bedwars::game::TeamId TeamManager::get_smallest_team() const {
    if (teams_.empty()) return bedwars::game::Teams::None;
    
    bedwars::game::TeamId smallest = teams_[0].id;
...
bedwars::game::TeamId TeamManager::get_winner() const {
    bedwars::game::TeamId winner = bedwars::game::Teams::None;
    ...
    return (aliveCount == 1) ? winner : bedwars::game::Teams::None;
}
